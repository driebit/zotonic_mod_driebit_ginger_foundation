...
{% if m.modules.active.mod_geomap %}
    {# mapsVersion: m.config.site.google_maps_version.value #}
    {% with "quarterly" as mapsVersion %}
        <script nonce="{{ m.req.csp_nonce }}" src="https://maps.googleapis.com/maps/api/js?key={{ m.geomap.google_api_key|escape }}&amp;libraries=places&amp;language=nl&amp;v={{ mapsVersion }}"></script>
    {% endwith %}
    {% lib
        "js/vendors/markerclusterer.js"
        "js/vendors/infobox.js"
        "js/map.js"
    %}

    {% comment %}
        This triggers unsafe-eval exception:
