        {% if id.s.author %}
            <aside class="main-aside">
                {% with m.search[{query hasobject=[id,'author'] cat_exclude=m.modules.enabled|index_of:"mod_ginger_remark"|if:"remark":"" pagelen=6 sort='-rsc.modified'}] as result %}
        ...
        {% if id.o.interest %}
            <aside class="main-aside">
                {% with m.search[{query hassubject=[id,'interest'] pagelen=6 sort='-rsc.modified'}] as result %}
        ...
        {% if m.modules.enabled|index_of:"mod_ginger_remark" %}
            {% with m.search[{query cat="remark" hasobject=[id,'author'] pagelen=6 sort='-rsc.modified'}] as result %}
